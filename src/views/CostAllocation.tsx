...
export default function CostAllocation({ token, canManage }: { readonly token: string; readonly canManage: boolean }) {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { if (form.get('action') === 'preview') { await previewRule(rule); return; } await createCostAllocationRule(token, rule); setCreating(false); await load(); }
  ...
  return <div className="space-y-6">
    ...
    {closures.length > 0 && <section className="rounded-xl border border-emerald-800/60 bg-emerald-950/20 p-4"><h2 className="font-bold">Cierres reproducibles de {period}</h2><p className="mt-1 text-sm text-zinc-400">Cada moneda se cierra por separado. Un reemplazo conserva la versión anterior para auditoría.</p><div className="mt-3 space-y-2">{closures.map((closure) => <div key={closure.id} className="flex flex-wrap justify-between gap-3 rounded border border-zinc-800 p-3 text-sm"><span><b>{closure.currency}</b> · versión {closure.version} · {closure.status === 'CLOSED' ? 'Cerrado' : 'Reemplazado'}<small className="ml-2 text-zinc-500">Fuente {closure.sourceHash.slice(0, 10)}…</small></span><span>Total {closure.sourceTotal.toFixed(2)} · asignado {closure.allocatedTotal.toFixed(2)} · sin asignar {closure.unallocatedTotal.toFixed(2)}</span></div>)}</div></section>}
    ...
    <section className="rounded-xl border border-zinc-800 bg-zinc-900 p-4"><h2 className="font-bold">Reglas</h2>{rules.length === 0 ? <p className="mt-3 text-zinc-500">No hay reglas configuradas. Cree una regla para asignar el gasto sin etiqueta.</p> : <div className="mt-3 space-y-2">{rules.map((rule) => <div key={rule.id} className="flex flex-wrap justify-between gap-3 border-b border-zinc-800 py-3"><div><b>{rule.name}</b> <span className="text-xs text-zinc-500">Prioridad {rule.priority} · {labelStatus(rule.status)} · {rule.allocationMode === 'SPLIT' ? `Distribución ${rule.allocationTargets.map((item) => `${item.percentage}%`).join(' + ')}` : 'Directa'} · v{rule.configurationVersion}</span><p className="text-sm text-zinc-400">{criterion(rule)} → {target(rule)}</p></div>{canManage && <div className="flex flex-wrap gap-3 text-sm"><button onClick={() => void previewRule(toCostAllocationInput(rule))}>Previsualizar</button><button onClick={() => void editRule(rule)}>Editar</button>{rule.status === 'DRAFT' && <button onClick={() => void activateCostAllocationRule(token, rule.id).then(load).catch((cause) => setError(message(cause, 'No fue posible activar la regla')))}>Activar</button>}{rule.status !== 'ARCHIVED' && <button className="text-red-300" onClick={() => void archiveCostAllocationRule(token, rule.id).then(load).catch((cause) => setError(message(cause, 'No fue posible archivar la regla')))}>Archivar</button>}</div>}</div>)}</div>}</section>
