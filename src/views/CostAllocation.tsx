...
export default function CostAllocation({ canManage }: { readonly canManage: boolean }) {
  ...
  return <div className="ui-page space-y-6">
    <header className="ui-page-header"><div><p className="ui-kicker">Gobierno financiero</p><h1 className="ui-page-title mt-2">Asignación de costos</h1><p className="ui-page-lead">Showback determinístico por reglas, dimensiones FOCUS y etiquetas.</p></div><div className="flex flex-wrap gap-2"><input aria-label="Periodo" type="month" value={period} onChange={(event) => setPeriod(event.target.value)} className={fieldClass}/><button onClick={() => void exportCsv()} className="ui-button ui-button-secondary">Exportar CSV</button>{canManage && <button onClick={() => setCreating((open) => !open)} className="ui-button ui-button-primary">Nueva regla</button>}</div></header>
    {options?.latestPeriod !== undefined && options.latestPeriod !== period && <div className="ui-callout ui-callout-accent flex flex-wrap items-center justify-between gap-3 p-4 text-sm text-tak-yellow"><span>El último período con costos disponibles es {options.latestPeriod}. Este período puede no tener datos.</span><button onClick={() => setPeriod(options.latestPeriod!)} className="ui-button ui-button-secondary min-h-9 px-3 text-xs">Usar período disponible</button></div>}
    ...
    {error !== null && <p className="ui-alert-danger p-3">{error}</p>}
...
const fieldClass = 'ui-control p-2 outline-none';
