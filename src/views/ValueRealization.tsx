...
export default function ValueRealization({ canReconcile, onOpenRecommendation }: Props) {
  ...
  return <div className="ui-page space-y-6 animate-fade-in">
    <header className="ui-page-header">
      <div><p className="ui-kicker">Resultado después de ejecutar</p><h1 className="ui-page-title mt-2">Valor realizado</h1><p className="ui-page-lead">Compara lo estimado con lo observado y lo verificado. Los aumentos de costo se muestran separados del ahorro.</p></div>
      <div className="flex flex-wrap gap-2"><button onClick={() => void load(false)} className="ui-button ui-button-secondary">Actualizar</button>{canReconcile && <button disabled={busy} onClick={() => void runReconcile()} className="ui-button ui-button-primary">{busy ? 'Actualizando…' : 'Actualizar mediciones'}</button>}<button onClick={() => void exportCsv()} className="ui-button ui-button-secondary">Exportar CSV</button></div>
    </header>
    ...
    {summary !== null && <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{summary.currencies.map((currency) => <div key={currency.currency} className="ui-surface-raised p-4"><div className="flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-wide text-zinc-500">Resumen {currency.currency}</p><span className="text-xs text-emerald-300">{(currency.realizationRate * 100).toFixed(1)}% realizado</span></div><div className="mt-3 grid grid-cols-2 gap-3 text-sm"><Metric label="Estimado" value={money(currency.estimatedMonthlySavings, currency.currency)} /><Metric label="Reportado" value={money(currency.reportedMonthlySavings, currency.currency)} /><Metric label="Observado" value={money(currency.observedSavings, currency.currency)} /><Metric label="Proyectado mensual" value={money(currency.projectedMonthlySavings, currency.currency)} tone="text-sky-200" /><Metric label="Run-rate verificado" value={money(currency.verifiedMonthlySavings, currency.currency)} tone="text-tak-yellow" /><Metric label="Aumento" value={money(currency.costIncreaseMonthlyAmount, currency.currency)} tone="text-rose-300" /><Metric label="Variación" value={money(currency.varianceAgainstEstimate, currency.currency)} /></div></div>)}</section>}
    <section className="ui-surface p-5"><div className="flex items-center justify-between"><div><h2 className="font-bold text-white">Cola de trabajo</h2><p className="text-xs text-zinc-500">Prioriza aumentos, revisiones y ejecuciones sin evidencia posterior.</p></div><span className="text-xs text-zinc-500">{attentionItems(items).length} pendientes visibles</span></div>{attentionItems(items).slice(0, 5).length === 0 ? <Empty text="No hay mediciones que requieran atención inmediata." /> : <div className="mt-4 space-y-2">{attentionItems(items).slice(0, 5).map((item) => <div key={`attention-${item.recommendationId}`} className="ui-surface-raised flex flex-wrap items-center justify-between gap-3 p-3"><div><p className="font-medium text-white">{attentionReason(item)}</p><p className="text-xs text-zinc-500">{item.title} · {item.resourceId ?? item.serviceName ?? 'Cuenta completa'}</p></div><div className="flex items-center gap-3"><span className="text-sm font-semibold text-zinc-200">{money(item.costIncreaseMonthlyAmount > 0 ? item.costIncreaseMonthlyAmount : item.estimatedMonthlySavings, item.currency)}</span><button onClick={() => onOpenRecommendation(item.recommendationId)} className="ui-button ui-button-quiet min-h-9 px-0 text-xs">Revisar recomendación</button></div></div>)}</div>}</section>
    <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
       <div className="ui-surface p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-bold text-white">Evolución del valor</h2><p className="text-xs text-zinc-500">Observado en la ventana, run-rate mensual verificado y aumentos separados</p></div><span className="text-xs text-zinc-500">{primaryCurrency} · {trend.length} periodos</span></div>{trend.length > 0 ? <ValueRealizationTrendUPlot points={trend} currency={filters.currency ?? primaryCurrency} /> : <Empty text="Aún no hay mediciones para graficar." />}</div>
      <div className="ui-surface p-5"><h2 className="font-bold text-white">Embudo de realización</h2><div className="mt-4 space-y-3">{[['Identificadas', count('identified')], ['Aprobadas', count('approved')], ['Ejecutadas', count('executed')], ['Sin medición', count('withoutMeasurement')], ['En revisión', count('calculatedPendingReview')], ['Verificadas', count('verified')]].map(([label, value]) => <div key={String(label)} className="flex items-center justify-between border-b border-zinc-800 pb-2 text-sm"><span className="text-zinc-400">{label}</span><strong className="text-white">{value}</strong></div>)}</div></div>
    </section>
      <section className="ui-surface p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold text-white">Portafolio de oportunidades</h2><p className="text-xs text-zinc-500">Una fila por recomendación y su última ejecución medible.</p></div><div className="flex flex-wrap gap-2"><input value={filters.search ?? ''} onChange={(event) => updateFilter('search', event.target.value)} placeholder="Buscar recurso o oportunidad" className="ui-control w-56 px-3 py-2 text-sm outline-none" /><select value={filters.currency ?? ''} onChange={(event) => updateFilter('currency', event.target.value)} className="ui-control px-3 py-2 text-sm outline-none"><option value=''>Todas las monedas</option>{(summary?.currencies ?? []).map((item) => <option key={item.currency} value={item.currency}>{item.currency}</option>)}</select><select value={filters.status ?? ''} onChange={(event) => updateFilter('status', event.target.value)} className="ui-control px-3 py-2 text-sm outline-none"><option value="">Todos los estados</option>{Object.entries(statusLabels).filter(([key]) => key !== 'NO_EXECUTION').map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><label className="ui-surface-raised flex items-center gap-2 px-3 py-2 text-xs text-zinc-300"><input type="checkbox" checked={filters.onlyIncreases === true} onChange={(event) => updateFilter('onlyIncreases', event.target.checked)} /> Aumentos</label></div></div>
      ...
      {hasMore && <div className="mt-4 text-center"><button onClick={() => void load(true)} disabled={loading} className="ui-button ui-button-secondary disabled:opacity-50">{loading ? 'Cargando…' : 'Cargar más'}</button></div>}
    </section>
  </div>;
}

function Kpi({ label, value, tone = 'text-white' }: { readonly label: string; readonly value: string; readonly tone?: string }) { return <div className="ui-surface-raised p-4"><p className="ui-label">{label}</p><p className={`mt-2 text-xl font-bold ${tone}`}>{value}</p></div>; }
