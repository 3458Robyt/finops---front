...
export default function ValueRealization({ token, canReconcile, onOpenRecommendation }: Props) {
  ...
  const runReconcile = async () => { setBusy(true); try { await reconcileValueRealization(token); await load(false); } catch (cause) { setError(cause instanceof Error ? cause.message : 'No fue posible reconciliar'); } finally { setBusy(false); } };
  ...
  return <div className="space-y-6 animate-fade-in">
    <header className="flex flex-wrap items-end justify-between gap-4">
      ...
      <div className="flex gap-2"><button onClick={() => void load(false)} className="rounded-lg border border-zinc-700 px-4 py-2 text-sm text-zinc-200 hover:border-tak-yellow">Actualizar</button>{canReconcile && <button disabled={busy} onClick={() => void runReconcile()} className="rounded-lg bg-tak-yellow px-4 py-2 text-sm font-bold text-zinc-950 disabled:opacity-50">{busy ? 'Conciliando…' : 'Conciliar mediciones'}</button>}<button onClick={() => void exportCsv()} className="rounded-lg border border-zinc-700 px-4 py-2 text-sm text-zinc-200 hover:border-tak-yellow">Exportar CSV</button></div>
    </header>
    ...
    {summary !== null && summary.currencies.length > 1 && <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{summary.currencies.map((currency) => <div key={currency.currency} className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><div className="flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-wide text-zinc-500">Resumen {currency.currency}</p><span className="text-xs text-emerald-300">{(currency.realizationRate * 100).toFixed(1)}% realizado</span></div><div className="mt-3 grid grid-cols-3 gap-3 text-sm"><div><p className="text-xs text-zinc-500">Estimado</p><p className="mt-1 font-bold text-white">{money(currency.estimatedMonthlySavings, currency.currency)}</p></div><div><p className="text-xs text-zinc-500">Verificado</p><p className="mt-1 font-bold text-tak-yellow">{money(currency.verifiedMonthlySavings, currency.currency)}</p></div><div><p className="text-xs text-zinc-500">Aumento</p><p className="mt-1 font-bold text-rose-300">{money(currency.costIncreaseMonthlyAmount, currency.currency)}</p></div></div></div>)}</section>}
    <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-bold text-white">Evolución del valor</h2><p className="text-xs text-zinc-500">Solo mediciones verificadas y aumentos detectados</p></div><span className="text-xs text-zinc-500">{trend.length} periodos</span></div>{trend.length > 0 ? <ValueRealizationTrendUPlot points={trend} /> : <Empty text="Aún no hay mediciones para graficar." />}</div>
