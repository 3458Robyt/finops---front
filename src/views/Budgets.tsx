...
export default function Budgets({ token, canManage }: { readonly token: string; readonly canManage: boolean }) {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await createBudget(token, { scope, period, amount: Number(form.get('amount')), currency: String(form.get('currency')).toUpperCase(), ...(scope === 'SERVICE' ? { serviceName: String(form.get('serviceName')) } : {}), ...(scope === 'CLOUD_ACCOUNT' ? { cloudAccountId: String(form.get('cloudAccountId')) } : {}) });
      setCreating(false); await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'No fue posible crear el presupuesto'); }
  };

  const editAmount = async (budget: Budget) => {
    const value = window.prompt('Nuevo importe mensual', String(budget.amount));
    if (value === null) return;
    const amount = Number(value);
    if (!Number.isFinite(amount) || amount <= 0) { setError('El importe debe ser positivo.'); return; }
    try { await updateBudget(token, budget.id, { amount }); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'No fue posible editar el presupuesto'); }
  };
