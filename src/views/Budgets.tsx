...
export default function Budgets({ token, canManage }: { readonly token: string; readonly canManage: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <p className="text-zinc-400">Cargando presupuestos…</p> : budgets.length === 0 ? <div className="rounded-xl border border-dashed border-zinc-700 p-10 text-center text-zinc-400">No hay presupuesto real configurado para este periodo.</div> : <div className="grid gap-4">{budgets.map((budget) => <BudgetCard key={budget.id} budget={budget} performance={performance[budget.id]} alerts={alerts[budget.id] ?? []} canManage={canManage} onEvaluate={() => void evaluateBudgets(token, budget.id).then(load)} onEdit={() => void editBudget(budget)} onArchive={() => void archiveBudget(token, budget.id).then(load)} />)}</div>}
  </div>;
}
