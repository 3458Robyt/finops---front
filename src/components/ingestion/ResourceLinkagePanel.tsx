...
function CoverageCard({ label, value, detail }: { readonly label: string; readonly value: string; readonly detail: string }) { return <div className="ui-surface-raised p-4"><p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">{label}</p><p className="mt-2 text-2xl font-black text-white">{value}</p><p className="mt-1 text-xs text-zinc-500">{detail}</p></div>; }
function ClassificationCard({ label, value, tone, detail }: { readonly label: string; readonly value: number; readonly tone: 'green' | 'sky' | 'yellow' | 'zinc'; readonly detail?: string }) { const tones = { green: 'text-green-300', sky: 'text-sky-300', yellow: 'text-tak-yellow', zinc: 'text-zinc-300' }; return <div className="ui-surface-raised p-4"><p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">{label}</p><p className={`mt-2 text-2xl font-black ${tones[tone]}`}>{value}</p>{detail !== undefined && <p className="mt-1 text-xs text-zinc-500">{detail}</p>}</div>; }
