...
export function AgentSettingsGovernance({ canConfigureAgent, saving, profile, form, ruleForm, activeRules, onFormChange, onRuleFormChange, onActivateProfile, onCreateRule, onDisableRule }: AgentSettingsGovernanceProps) {
  ...
  return <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
    ...
    <div className="space-y-4">
      {canConfigureAgent && <div className="ui-surface space-y-4 p-5"><SectionHeader title="Nueva regla tenant" eyebrow="Contexto local" icon="add_task" /><Input label="Categoria" value={ruleForm.category} onChange={(value) => onRuleFormChange({ ...ruleForm, category: value })} /><Input label="Prioridad" value={ruleForm.priority} onChange={(value) => onRuleFormChange({ ...ruleForm, priority: value })} /><TextArea label="Regla del cliente" value={ruleForm.ruleText} rows={5} onChange={(value) => onRuleFormChange({ ...ruleForm, ruleText: value })} /><button onClick={onCreateRule} disabled={saving} className="ui-button ui-button-primary w-full disabled:opacity-60">Agregar regla</button></div>}
      <div className="ui-surface space-y-3 p-5"><SectionHeader title="Reglas activas" eyebrow="Tenant" icon="rule" />{activeRules.length === 0 ? <p className="text-sm font-bold text-zinc-500">No hay reglas activas del cliente.</p> : activeRules.map((rule) => <article key={rule.id} className="ui-surface-raised p-4"><p className="text-[10px] font-black uppercase tracking-widest text-tak-yellow">{rule.category} - prioridad {rule.priority}</p><p className="mt-2 text-sm leading-relaxed text-zinc-200">{rule.ruleText}</p>{canConfigureAgent && <button onClick={() => onDisableRule(rule.id)} disabled={saving} className="ui-button ui-button-quiet mt-3 min-h-9 px-3 text-xs disabled:opacity-60">Desactivar</button>}</article>)}</div>
    </div>
  </section>;
}
