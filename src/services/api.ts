...
export interface AllocationSummary { readonly period: string; readonly currency: string; readonly totalCost: number; readonly allocatedCost: number; readonly unallocatedCost: number; readonly coveragePercent: number; readonly dimensions: readonly { readonly allocationKey: string; readonly cost: number; readonly metricCount: number; readonly resourceCount: number }[]; }
export interface AllocationPreview { readonly summary: readonly AllocationSummary[]; readonly metricCount: number; readonly resourceCount: number; readonly examples: readonly { readonly currency: string; readonly cost: number; readonly cloudAccountId: string; readonly serviceName: string; readonly resourceId?: string }[]; }
...
export async function fetchCostAllocationSummary(token: string, period: string, filters: { readonly cloudAccountId?: string; readonly serviceName?: string } = {}): Promise<{ readonly success: true; readonly summary: readonly AllocationSummary[] }> { const query = new URLSearchParams({ period }); if (filters.cloudAccountId !== undefined) query.set('cloudAccountId', filters.cloudAccountId); if (filters.serviceName !== undefined) query.set('serviceName', filters.serviceName); return apiRequest(`/cost-allocation/summary?${query}`, { token }); }
export async function fetchCostAllocationComparison(token: string, period: string, filters: { readonly cloudAccountId?: string; readonly serviceName?: string } = {}): Promise<{ readonly success: true; readonly comparison: { readonly summary: readonly AllocationSummary[]; readonly previousSummary: readonly AllocationSummary[] } }> { const query = new URLSearchParams({ period }); if (filters.cloudAccountId !== undefined) query.set('cloudAccountId', filters.cloudAccountId); if (filters.serviceName !== undefined) query.set('serviceName', filters.serviceName); return apiRequest(`/cost-allocation/comparison?${query}`, { token }); }
export async function fetchUnallocatedCosts(token: string, period: string, filters: { readonly currency?: string; readonly cloudAccountId?: string; readonly serviceName?: string } = {}): Promise<{ readonly success: true; readonly items: readonly { readonly cost: number; readonly currency: string; readonly serviceName: string; readonly cloudAccountId: string; readonly resourceId?: string; readonly suggestedCriteria: readonly string[] }[] }> { const query = new URLSearchParams({ period }); if (filters.currency !== undefined) query.set('currency', filters.currency); if (filters.cloudAccountId !== undefined) query.set('cloudAccountId', filters.cloudAccountId); if (filters.serviceName !== undefined) query.set('serviceName', filters.serviceName); return apiRequest(`/cost-allocation/unallocated?${query}`, { token }); }
